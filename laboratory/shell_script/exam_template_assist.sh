...
function usage {
  echo "exam_template.sh のための csv 作成スクリプト(MacOS／WSL環境対応)"
...
function exam_template_assist () {
  while IFS= read -r line || [[ -n $line ]]; do # テキストファイルを1行ずつ読み込んで処理する
    trimmed_line=$(echo "$line" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//') # 先頭と末尾の空白をトリムする
    if [ -z "$trimmed_line" ]; then # 空行の場合もCSVの空行として出力する
      echo >> "$main_file"
    else
      echo -n "$trimmed_line," >> "$main_file" # カンマで区切ってCSV形式の1行に書き出す
    fi
  done < "$file_path"
