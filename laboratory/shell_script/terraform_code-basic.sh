...
first_line=$(head -n 1 "$src_file") # テキストファイルの1行目を読み込む

function usage () {
  echo "変数用ファイルを使って Terraform の基本的なファイルを出力するスクリプト"
  echo "入力方法: $this [ソースファイルパス] [出力ディレクトリ名]"
  exit 1
}
...
if [ ! -e "$1" ] || [ ! -f "$1" ]; then
  ...
  echo -e "\033[1;31mERROR: 指定されたソースファイル $main_file は無効です。\033[0m"
  echo -e "\033[1;31mERROR: ソースファイル $main_file の一行目に適切な修正を行って再度実行してください。\033[0m"
  exit 1
else
  echo -e "\033[1;32mSUCCESE: ソースファイルパス $src_file は有効です。\033[0m"
fi

if [ -z "$2" ]; then
  echo -e "\033[1;31mERROR: 出力ディレクトリ名が指定されていません。出力ディレクトリ名を入力して再度実行してください。\033[0m"
  echo -e "\033[1;31mEXAMPLE: terraform-aws\033[0m"
  exit 1
else
  echo -e "\033[1;32mSUCCESE: 出力ディレクトリ名は $dir_name です。\033[0m"
fi
...
if [[ $first_line =~ "# variable" ]] && [ -n "$2" ]; then
