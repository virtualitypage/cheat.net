...
EOF
  ...
  configure=$(perl -pe 'chomp if eof' <<< "$configure")
  echo "$configure" | perl -pe 'chomp if eof' >> "$config"
  echo
...
function terraform_uninstall () {
  ...
  # ~/.zshrc の該当行を削除
  echo "sed -i '' -e '/complete -o nospace -C \/usr\/local\/bin\/terraform terraform/d' $HOME/.zshrc"
  if grep -q 'complete -o nospace -C /usr/local/bin/terraform terraform' "$HOME/.zshrc"; then
    sed -i '' -e '/complete -o nospace -C \/usr\/local\/bin\/terraform terraform/d' "$HOME/.zshrc"
  ...
  # ~/.bash_profile の該当行を削除
  echo "sed -i '' -e '/complete -C \/usr\/local\/bin\/terraform terraform/d' $HOME/.bash_profile"
  if grep -q 'complete -C /usr/local/bin/terraform terraform' "$HOME/.bash_profile"; then
    sed -i '' -e '/complete -C \/usr\/local\/bin\/terraform terraform/d' "$HOME/.bash_profile"
