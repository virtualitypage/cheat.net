...
dirs="/Volumes/Internal/var/cache/*"
src_volume="/Volumes/Internal/var/cache"
src_volume_path="/Volumes/Internal/var/cache/$today"
...
function rsync_clone_folder () {
  ...
  echo
  echo -e "\033[1;32mSERVER \"$SERVER\" から以下のフォルダが検出されました\033[0m"
  for d in $dirs; do
    dirname=$(basename "$d")
...
function automator_rsync_google_drive () {
  ...
  if [ -e "$src_volume/${today}_$urandom.zip" ]; then
    echo -e "\033[1;36mINFO: SERVER \"$SERVER\" から \"${today}_$urandom.zip\" を削除しています…\033[0m"
    echo "rm -v $src_volume/${today}_$urandom.zip"
    rm -v $src_volume/"$today"_"$urandom".zip
    echo
    echo -e "\033[1;32mSUCCESS: SERVER \"$SERVER\" から \"${today}_$urandom.zip\" を削除しました\033[0m"
    echo
  fi
...
if [ -e $src_volume ]; then
  ...
  echo -e "\033[1;31mERROR: SERVER \"$SERVER\" と $dst_volume が存在しません。ドライブのマウント、およびサーバーへのアクセス可否を確認して再度実行してください。\033[0m"
  echo
  exit 1
fi

if [ -d "$src_volume_path" ]; then
  echo -e "\033[1;31mERROR: SERVER \"$SERVER\" に転送用の一時フォルダが存在しません。転送用の一時フォルダを作成・転送プロセスに移行します。\033[0m"
  rsync_clone_folder
  mv_google_drive
elif [ ! -e "$src_volume_path" ]; then
  echo -e "\033[1;31mERROR: SERVER \"$SERVER\" に転送用フォルダ \"$today\" が存在しません。転送用フォルダを作成・転送プロセスに移行します。\033[0m"
  rsync_clone_folder
  mv_google_drive
else
  echo -e "\033[1;33mWARNING: SERVER \"$SERVER\" に \"$today\" という拡張子を持たない不正なファイルがあります。\033[0m"
