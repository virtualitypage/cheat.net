...
function convert_to_csv() {
    while IFS= read -r string_column; do
        ...
        echo "$csv_line" >> "${current_dir}/$CSV_FILE"
    ...
    # 合計値を最終行の1列目に追加
    echo "合計額,$total" >> "$CSV_FILE"

    # 予算額を最終行の2列目に追加
    echo "予算(50000円),$((50000 - total))" >> "${current_dir}/$CSV_FILE"
}

if [ -f "${current_dir}/${FILE}" ]; then
  convert_to_csv
  echo "計算およびファイル作成が完了しました。ファイルは${current_dir}/${CSV_FILE}に保存されました。"
  echo ""
