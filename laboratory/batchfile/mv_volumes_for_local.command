...
function mv_volumes () {
  date_dir=$(stat -f "%Sm" -t "%Y-%-m-%-d" $src_dir/$src_file)
  main_file="$dst_dir/$date_dir status.txt"
  cd $dst_dir || exit
  ...
  if [ $RETRY_COUNT -ge 3 ]; then
    ...
    echo -e "\033[1;31m       ・プログラムと実行環境のディレクトリパス \"$src_dir\" \"$dst_dir\" に齟齬が無い\033[0m"
  ...
  if [ "$files_found_mp4" = true ]; then
    ...
    for mp4_file in "${mp4_files[@]}"; do
      ...
      mp4_stat=$(stat -f "%Sm" -t "%Y年%m月%d日 %H:%M" "$dst_dir/$date_dir/$mp4_file")
  ...
  if [ "$files_found_mov" = true ]; then
    ...
    for mov_file in "${mov_files[@]}"; do
      ...
      mov_stat=$(stat -f "%Sm" -t "%Y年%m月%d日 %H:%M" "$dst_dir/$date_dir/$mov_file")
      if [ "$first_file" = true ]; then
  ...
  if [ "$files_found_avi" = true ]; then
    ...
    for avi_file in "${avi_files[@]}"; do
      ...
      avi_stat=$(stat -f "%Sm" -t "%Y年%m月%d日 %H:%M" "$dst_dir/$date_dir/$avi_file")
      if [ "$first_file" = true ]; then
  ...
  echo -e "\033[1;32mDISK \"$DISK\" 内のファイルは $dst_dir/$date_dir に格納されています。\033[0m"
  echo
  stream_editor
  end_point
}

function mv_volumes2 () {
  date_dir=$(stat -f "%Sm" -t "%Y-%-m-%-d" $src_dir2/$src_file)
  main_file="$dst_dir/$date_dir status.txt"
  cd $dst_dir || exit
  ...
  RETRY_COUNT=0
  while [ $RETRY_COUNT -lt 3 ]; do
    echo "rsync --archive --human-readable --progress $src_dir2/* $dst_dir/$date_dir"
    if rsync --archive --human-readable --progress $src_dir2/* "$dst_dir/$date_dir"; then
      echo
      read -rp "DISK \"$DISK\" 内のファイルを削除しますか？ { yes | y | no }: " yesno
      if [ "$yesno" = "yes" ] || [ "$yesno" = "y" ] || [ "$yesno" = "Y" ]; then
        echo "rm -i $src_dir2/*"
  ...
  if [ $RETRY_COUNT -ge 3 ]; then
    ...
    echo -e "\033[1;31m       ・転送元である DISK \"$DISK\" が挿入されている\033[0m"
    echo -e "\033[1;31m       ・転送先である DISK \"$DISK2\" が挿入されている\033[0m"
    echo -e "\033[1;31m       ・プログラムと実行環境のディレクトリパス \"$src_dir2\" \"$dst_dir\" に齟齬が無い\033[0m"
    echo -e "\033[1;31m       ・DISK \"$DISK\" 内に動画ファイルが存在している\033[0m"
    exit 1
  fi
