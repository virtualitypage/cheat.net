...
for h in {0..23}; do
  for m in {0..5}; do
    ...
    sort --unique "$stat_file" | sed 's/$/, /g' > "$uniq_file"
    count=$(wc -l "$stat_file" | awk '{ print $1 }')
    uniq_query=$(< "$uniq_file" tr -d '\n' | sed -e 's/, $//g' -e 's/^/"/g' -e 's/$/"/g')
...
# 個数が 1 のドメインをまとめる
if grep --extended-regexp "^1:" "$counter.txt" 1>/dev/null; then
  echo -e "$code_15inch\n" > "$main_file_15inch"
  echo -e "$code_27inch\n" > "$main_file_27inch"
  grep --extended-regexp "^1:" "$counter.txt" > "${counter}_A.txt"
...
# 個数が 2 のドメインをまとめる
if grep --extended-regexp "^2:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^2:" "$counter.txt" > "${counter}_A.txt"
  sed -i '' 's/variable/3/g' "$main_file_15inch" "$main_file_27inch"
  sed -i '' -e '1s/.*: /set TargetDomain3 to {"/g' -e 's/2: /"/g' -e 's/$/", /g' "${counter}_A.txt"
  sed -i '' -e '$a \
  }' -e 's/,   //g' "${counter}_A.txt"
  < "${counter}_A.txt" tr -d '\n' > "${counter}_B.txt"
  sed -i '' 's/,   }/}\n\ntargetQuery_3(TargetDomain3)\n/g' "${counter}_B.txt"
  cat "${counter}_B.txt" >> "$main_file_15inch"
  cat "${counter}_B.txt" >> "$main_file_27inch"
fi

# 個数が 3 のドメインをまとめる
if grep --extended-regexp "^3:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  ...
  < "${counter}_A.txt" tr -d '\n' > "${counter}_B.txt"
  sed -i '' 's/,   }/}\n\ntargetQuery_4(TargetDomain4)\n/g' "${counter}_B.txt"
  cat "${counter}_B.txt" >> "$main_file_15inch"
  cat "${counter}_B.txt" >> "$main_file_27inch"
fi

# 個数が 4 のドメインをまとめる
if grep --extended-regexp "^4:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^4:" "$counter.txt" > "${counter}_A.txt"
  sed -i '' 's/variable/5/g' "$main_file_15inch" "$main_file_27inch"
  sed -i '' -e '1s/.*: /set TargetDomain5 to {"/g' -e 's/4: /"/g' -e 's/$/", /g' "${counter}_A.txt"
  sed -i '' -e '$a \
  }' -e 's/,   //g' "${counter}_A.txt"
  < "${counter}_A.txt" tr -d '\n' > "${counter}_B.txt"
  sed -i '' 's/,   }/}\n\ntargetQuery_5(TargetDomain5)\n/g' "${counter}_B.txt"
  cat "${counter}_B.txt" >> "$main_file_15inch"
  cat "${counter}_B.txt" >> "$main_file_27inch"
fi

# 個数が 5 のドメインをまとめる
if grep --extended-regexp "^5:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^5:" "$counter.txt" > "${counter}_A.txt"
  sed -i '' 's/variable/6/g' "$main_file_15inch" "$main_file_27inch"
  sed -i '' -e '1s/.*: /set TargetDomain6 to {"/g' -e 's/5: /"/g' -e 's/$/", /g' "${counter}_A.txt"
  sed -i '' -e '$a \
  }' -e 's/,   //g' "${counter}_A.txt"
   < "${counter}_A.txt" tr -d '\n' > "${counter}_B.txt"
...
# 個数が 6〜9 のドメインをまとめる
if grep --extended-regexp "^[6-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[6-9]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[1][0-5]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[1][0-5]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[1][6-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[1][6-9]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[2][0-5]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[2][0-5]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[2][6-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[2][6-9]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[3][0-5]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[3][0-5]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[3][6-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[3][6-9]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[4][0-5]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[4][0-5]:" "$counter.txt" > "${counter}_A.txt"
...
# 個数が 46〜49 のドメインを追加
if grep --extended-regexp "^[4][6-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[4][6-9]:" "$counter.txt" > "${counter}_A.txt"
...
# 個数が 50〜99 のドメインを追加
if grep --extended-regexp "^[5-9][0-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[5-9][0-9]:" "$counter.txt" > "${counter}_A.txt"
...
if grep --extended-regexp "^[1-9][0-9][0-9]:" "$counter.txt" 1>/dev/null; then
  echo >> "$main_file_15inch"
  echo >> "$main_file_27inch"
  echo -e "$code_15inch\n" >> "$main_file_15inch"
  echo -e "$code_27inch\n" >> "$main_file_27inch"
  grep --extended-regexp "^[1-9][0-9][0-9]:" "$counter.txt" > "${counter}_A.txt"
