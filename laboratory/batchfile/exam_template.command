...
function txt_error () {
  echo "変換処理に失敗しました。"
  echo "${txt_file}にデータが無いため、読み込むことができません。"
  exit 1
}
...
do
  ...
  if [ -z "$trimmed_line" ]; then
    ...
    echo -n "$trimmed_line," >> "${current_dir}/${csv}"
  fi
done < "${current_dir}/${txt_file}"
echo "csvファイル作成が完了しました。${csv}は${current_dir}に保存されています。"
}

function create_dir () {
while IFS=, read -r col1 col2 col3 col4 col5 col6 col7 col8 col9 col10 _ || [[ -n $col10 ]];
do
...
if [ -f "${current_dir}/${csv}" ]; then
  create_dir
  echo "htmlを含むディレクトリを作成しました。"
  echo ""
  exit 0
fi

if [ -f "${current_dir}/${txt_file}" ]; then
  if [ -s "${current_dir}/${txt_file}" ]; then
    convert_to_csv
    echo ""
  else
    txt_error
